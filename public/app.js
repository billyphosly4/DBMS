...
function renderMetrics() {
  ...
  const cards = [['Active European Placements', applicants.length.toLocaleString(), 'LIVE', 'candidates in the placement control plane', 'globe-2'], ['Visa & Work Permit Queue', `${Math.max(applicants.length - verified, 0)}`, 'QUEUE', 'profiles awaiting immigration verification', 'file-check-2'], ['EU Compliance Score', `${complianceScore}%`, 'READY', 'document readiness across the EU pipeline', 'shield-check'], ['Total Agency Revenue Logged', `€${totalRevenue.toLocaleString('en-IE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 'EUR', 'service fees recorded across candidates', 'banknote'], ['Live sync status', applicantStream ? 'CONNECTED' : 'WAITING', 'STREAM', 'euro_applicants listener', 'radio-tower']];
...
function renderApplicants() {
  ...
  records.forEach((applicant) => {
    ...
    const ledgerCell = `<div class="ledger-cell"><span class="fee-tag">€${fee.toLocaleString('en-IE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span><span class="payment-badge ${paymentClass}">${escapeHtml(applicant.paymentStatus || 'Unpaid Deposit')}</span></div>`;
...
function configureEuroTransitShell() {
  ...
  destinationSelect.parentElement.insertAdjacentHTML('afterend', '<label>Service fee amount<input name="serviceFee" type="number" min="0" step="0.01" inputmode="decimal" placeholder="e.g. 1500.00"></label><label>Payment status<select name="paymentStatus"><option>Unpaid Deposit</option><option>Deposit Settled</option><option>Fully Paid</option></select></label><label>Language proficiency<select name="languageProficiency"><option>English - IELTS/TOEFL</option><option>German - B1/B2</option><option>No Certificate</option></select></label>');
