...
async function submitApplicant(event) { event.preventDefault(); const formElement = event.currentTarget; const form = new FormData(formElement); const button = $('#submit-applicant'); button.disabled = true; button.textContent = 'Ingesting profile...'; try { const response = await authenticatedFetch('/api/applicants', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Object.fromEntries(form.entries())) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || 'Unable to ingest applicant.'); formElement.reset(); closeDrawer(); showToast('Applicant profile ingested into Firestore.'); } catch (error) { alert(error.message); } finally { button.disabled = false; button.innerHTML = '<i data-lucide="upload-cloud"></i>Ingest applicant profile'; lucide.createIcons(); } }
